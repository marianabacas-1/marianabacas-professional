...
export default function EditTask(props) {
    ...
    useEffect(() => {
        if (props.taskToEdit) {
            ...
            if(props.taskToEdit.assignedTo && (props.users?.length > 0)) {
                const user = props.users.find(usr => usr.value === props.taskToEdit.assignedTo)
                setSelectedUser(user);
            }
            if(props.taskToEdit.category && (props.categories?.length > 0)) {
                const category = props.categories.find(usr => usr.value === props.taskToEdit.category)
